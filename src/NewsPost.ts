...
export class NewsPost extends SduiBaseClass {
    ...
    */
    ...
    public async delete(): Promise<void> {
        if (!this.entity?.can?.delete) throw new SduiError("Cannot delete this post")
    ...
    public async update(entity: INewsPost): Promise<NewsPost> {
        ...
        const result = await this.Axios.put<ISduiResponse<INewsPost>>(
            `/channels/news/${this.entity!.id}`, this.entity)
        this.entity = result.data.data;
        return this;
    }

    public async confirm(): Promise<NewsPost> {
        if (!this.entity?.can?.confirm) throw new SduiError("Cannot confirm this post");
        const result = await this.Axios.put<ISduiResponse<INewsPost>>(
    ...
    public async pin(): Promise<NewsPost> {
        if (!this.entity?.can?.pin) return Promise.reject(new SduiError("Cannot pin this post"));
        const result = await this.Axios.put<ISduiResponse<INewsPost>>(
    ...
    public async unpin(): Promise<NewsPost> {
        if (!this.entity?.can?.pin) return Promise.reject(new SduiError("Cannot unpin this post"));
        const result = await this.Axios.put<ISduiResponse<INewsPost>>(
            `/channels/news/${this.entity!.id}/pin`, {
            "is_pinned": false,
        })
    ...
    public getSurvey(): Survey {
        if (!this.entity?.survey) throw new SduiError("No survey attached to this post");
